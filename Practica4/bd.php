...
function getEvento($id, $mysqli)
{
    ...
    if (!$sentencia = $mysqli->prepare("SELECT etiquetas, nombre, fecha, organizador,descripcion,fotos FROM eventos WHERE id = ?")) {
    ...
    if ($res->num_rows > 0) {
        $row = $res->fetch_assoc();
        $evento = array('id'=>$id,'etiquetas'=>$row['etiquetas'],'nombre' => $row['nombre'], 'fecha' => $row['fecha'], 'organizador' => $row['organizador'], 'descripcion' => $row['descripcion'], 'img' => $row['fotos']);
    }
    return $evento;
}
...
function getEventos($mysqli)
{
    ...
    if (!$sentencia = $mysqli->prepare("SELECT id,nombre,fotos,fecha,organizador FROM eventos")) {
        echo "Falló la preparación: (" . $mysqli->errno . ") " . $mysqli->error;
    }
...
function getEventosPalabras($buscador,$mysqli){
    ...
    if (!$sentencia = $mysqli->prepare("SELECT id,nombre,fotos,fecha,organizador FROM eventos WHERE descripcion LIKE CONCAT('%',?,'%')")) {
        echo "Falló la preparación: (" . $mysqli->errno . ") " . $mysqli->error;
    }

    if (!$sentencia->bind_param("s", $buscador)) {
        echo "Falló la vinculación de parámetros: (" . $sentencia->errno . ") " . $sentencia->error;
    }
...
function modificarEvento($idEvento,$nombre,$organizador,$etiquetas,$fecha,$descripcion,$mysqli)
{

    if (!$sentencia = $mysqli->prepare("UPDATE eventos SET nombre = ?, organizador = ?, etiquetas = ?, fecha = ?, descripcion = ?  WHERE id = ?")) {
        echo "Falló la preparación: (" . $mysqli->errno . ") " . $mysqli->error;
    }
    if (!$sentencia->bind_param("sssssi",$nombre,$organizador,$etiquetas,$fecha,$descripcion,$idEvento)) {
        echo "Falló la vinculación de parámetros: (" . $sentencia->errno . ") " . $sentencia->error;
    }
...
function modificarEventoImg($idEvento,$nombre,$organizador,$etiquetas,$fecha,$descripcion,$imagenes,$mysqli)
{

    if (!$sentencia = $mysqli->prepare("UPDATE eventos SET nombre = ?, organizador = ?, etiquetas = ?, fecha = ?, descripcion = ?, fotos = ?  WHERE id = ?")) {
        echo "Falló la preparación: (" . $mysqli->errno . ") " . $mysqli->error;
    }
    if (!$sentencia->bind_param("ssssssi",$nombre,$organizador,$etiquetas,$fecha,$descripcion,$imagenes,$idEvento)) {
        echo "Falló la vinculación de parámetros: (" . $sentencia->errno . ") " . $sentencia->error;
    }
...
function nuevoEvento($nombre, $organizador, $etiquetas, $fecha, $descripcion, $imagenes, $mysqli){

    if (!$sentencia = $mysqli->prepare("INSERT INTO eventos (nombre, organizador, etiquetas, fecha, descripcion, fotos)  VALUES (?,?,?,?,?,?)")) {
        echo "Falló la preparación: (" . $mysqli->errno . ") " . $mysqli->error;
    }


    if (!$sentencia->bind_param("ssssss",$nombre, $organizador, $etiquetas, $fecha, $descripcion, $imagenes )) {
        echo "Falló la vinculación de parámetros: (" . $sentencia->errno . ") " . $sentencia->error;
    }
